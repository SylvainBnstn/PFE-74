...
class list_smart_clients:
    ...
    def wtp_actualisation(self, price, echeance,current_client):
        ...
        for i in range(range_echeance):
            if(echeance == (self.clients_list[current_client].echeance - i)):
    ...
    def check_sales(self, price, echeance, price_trace, list_resa):
    ...
    def update_client(self,prix_min,prix_max,list_to_del,resting_time):
        ...
        for _ in range(aleat):
            temp_min = rnd.uniform(prix_min,prix_min+(prix_max-prix_min)/2)
            temp_max = rnd.uniform(prix_min+(prix_max-prix_min)/2,prix_max)
            temp_wtp = rnd.random()
